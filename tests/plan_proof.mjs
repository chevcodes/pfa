...
// times too generous. Free spending must be what is actually spent freely.
...
note(groups.free.actual === 40000, 'free = only the guilt-free card spend, not the whole remainder');
note(
...
note(groupForCategory('Dining & Takeout', seeded) === 'free', 'dining is free spending, however often it recurs');
...
note(m.free.label === 'free spending allocation', 'the label uses the established plan term without editorial framing');
const carrying = buildPlanModel(
  buildPlan({ ...base, card: { owed: 60000, previousOwed: 40000 } }),
  CFG
);
note(/grew/.test(carrying.free.reconciling.text), 'a growing card balance surfaces as plain fact');
...
console.log('         reassigned; and free spending is what is actually spent freely.');
console.log('='.repeat(72));
