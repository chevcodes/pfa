...
note(
  /\bwarn\(/.test(branch) && !/state\.bankWarnings\.push\(/.test(branch),
  'its warnings go to the channel the app actually shows'
);
note(
  /const warn = \(text\) => state\.warnings\.push\(\{ file: file\.name/.test(intake),
  'and that channel records which file each warning is about'
);
note(
