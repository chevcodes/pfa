...
// The card IS the disclosure: its closed summary carries the total, the account
// count and the date, so opening it already means "show me the detail". A "See
// the detail" fold inside it made one tap reveal another tap.
note(
  !/'See the detail'/.test(render) &&
    /card\.append\(\s*historyDetail\(statements, snap, base\),\s*holdingsDetail\(/.test(render),
  'history and holdings sit directly in the card, behind no second door'
);
note(
