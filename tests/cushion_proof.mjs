...
console.log('\n -- the exact-figures rule for exports and the report holds --');
{
  ...
  const offenders = [];
  for (const f of readdirSync('application/output')) {
    if (!f.endsWith('.js')) continue;
    const src = readFileSync(`application/output/${f}`, 'utf8');
    if (/makeMoneyShort|makeMoneyCompact|moneyShort\s*\(/.test(src)) offenders.push(f);
  }
