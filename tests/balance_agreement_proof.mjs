...
/* EVERY FIGURE IN THE SHAREABLE SUMMARY IS DATED BY ITS OWN SOURCE.
 *
 * This is the one artefact built to be handed to somebody else, and it stamped
 * every line with the day it was produced - so a cash total whose largest
 * component had not been seen since a statement seven weeks earlier read "as of
 * today". The dates were already known inside liquidBalance and thrown away.
 * A sum of balances confirmed weeks apart has no single as-of date, so both
 * ends are stated. */
console.log('\n -- the shareable summary dates each figure by its own evidence --');
{
  const bank = [
    { account: 'A', date: '2026-07-24', seq: 1, currency: 'JMD', amount: 1, direction: 'out', balanceAfter: 1000000 },
    { account: 'B', date: '2026-08-31', seq: 1, currency: 'JMD', amount: 1, direction: 'out', balanceAfter: 120490.89 },
  ];
  const cd = cashAndDebt({
    bankRecords: bank,
    cardStatements: [{ newBalance: 152156.84, creditLimit: 660000, periodEnd: '2026-09-15', statementKey: '2026-09' }],
    asOf: '2026-09-20',
  });
  const nw = recordedNetWorth({ reconciled: cd, asOf: '2026-09-20' });
  const rows = financialPositionSummary({ cashDebt: cd, netWorth: nw, asOf: '2026-09-20' }).rows;
  const row = (label) => rows.find((r) => r.label === label);
  note(
    row('Cash on hand').period === 'as of 2026-07-24 to 2026-08-31',
    `cash spans the dates of the balances behind it (got "${row('Cash on hand').period}")`
  );
  note(
    row('Card balance').period === 'as of 2026-09-15',
    `the card is dated by its statement, not by today (got "${row('Card balance').period}")`
  );
  note(
    !rows.some((r) => /Cash|Card/.test(r.label) && r.period === 'as of 2026-09-20'),
    'and no statement-backed figure claims today'
  );

  const sameDay = cashAndDebt({
    bankRecords: [{ account: 'A', date: '2026-08-31', seq: 1, currency: 'JMD', amount: 1, direction: 'out', balanceAfter: 500 }],
    cardStatements: [],
    asOf: '2026-09-20',
  });
  const sameRows = financialPositionSummary({
    cashDebt: sameDay,
    netWorth: recordedNetWorth({ reconciled: sameDay, asOf: '2026-09-20' }),
    asOf: '2026-09-20',
  }).rows;
  note(
    sameRows.find((r) => r.label === 'Cash on hand').period === 'as of 2026-08-31',
    'one date when the balances agree, rather than a range repeating itself'
  );
}
