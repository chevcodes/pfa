...
for (const cls of ['dh-note', 'plan-note', 'plan-assign-name', 'proportion-legend-label', 'wiz-q']) {
...
note(
  /const range = `\$\{ctx\.monthShort\(model\.months\[0\]\)\}/.test(
...
console.log('\n -- one number, one source: no hand-rolled duplicates --');
{
  ...
  const DUPES = [
    ...
    [/0\.6745\s*\*/, 'a hand-rolled modified z-score constant (use modifiedZ from shared-helpers)'],
    [
      /r\.internalTransfer != null\s*\n?\s*\?\s*!!r\.internalTransfer/,
      'a hand-rolled internal-transfer reader (use isInternal from shared-helpers)',
    ],
  ];
  const offenders = [];
...
console.log('\n -- every choice carries a way back --');
{
  ...
  // planSetAside has retired: which accounts count as saving is now one of the
  // answers in the shared confirmation store, and its way back is proved in
  // tests/confirmations_proof.mjs along with every other answer's.
  const CHOICE_KEYS = ['planGroups', 'planTarget', 'financeGoal', 'financeGoalBoundary'];
...
console.log('\n -- notice and transaction filters keep content visible --');
{
  ...
  note(
    /\.install-banner\s*\{[^}]*max-width:\s*calc\(\s*var\(--workspace-max,/s.test(controls),
    'notice banners use the shared workspace width'
  );
  note(
    /name: 'category-filter',\s*compact: true,/.test(activity) && /wrap\.append\(searchFilters\)/.test(activity),
    'the category filter is the same compact card as the account filter above it'
  );
  note(
    /explain: categoryExplain/.test(activity) && !/tx-cats-label/.test(activity),
    'and its explanation is the card ⓘ, not a label of its own'
  );
  note(
  ...
  note(
    /#app \.card\.card-compact,\s*\.card\.card-compact \{\s*padding: 0;\s*\}/.test(additions) &&
      /\.card\.card-compact > \.card-disclosure > summary \{\s*padding-top: 8px;\s*padding-bottom: 8px;/.test(additions),
    'the account filter stays wide and shares every other card’s left edge; only its height is trimmed'
  );
}
...
console.log('\n -- one frame for "N out of M" --');
{
   ...
   *
   * accounts-render's count later moved off its own bordered tile entirely,
   * into the same `subhead` note every statement-trust panel uses, so it now
   * reads the exact phrase cards-render does - "N of M reconcile" - not just
   * the same shape. */
  ...
  note(
    ...
    'the account panel no longer states its ratio as a slash fraction'
  );
  note(
    /\$\{totalOk\} of \$\{totalN\} reconcile/.test(accounts),
    'it reads "N of M reconcile", exactly like the card panel above it'
  );
  note(
    /\$\{reconciled\} of \$\{stmts\.length\} reconcile/.test(cards) &&
   ...
   ------------------------------------------------------------------------ */
{
  ...
  note(
    /title: 'Expected payments',[\s\S]{0,260}summary: beforeIncome\.length[\s\S]{0,260}due within/.test(ahead) &&
      !/title: 'Expected payments',[\s\S]{0,120}open:/.test(ahead),
    'Expected payments stays closed and prioritises the payments before next income'
  );
...
console.log('\n -- a prose column has a floor, and a sub-heading has an ⓘ that works --');
{
  // `additions`, `controller` and `cards` are already read at file scope.
  const controls = readFileSync(join(ROOT, 'interface', 'controls.css'), 'utf8');
  const header = readFileSync(join(ROOT, 'application', 'ui', 'decision-header.js'), 'utf8');

  /* "The built-in categories / Match every transaction again, or reload..."
   * rendered ONE WORD PER LINE, a 43px grey ribbon beside its buttons: a
   * prose column with `flex: 1 1 180px` (an aim) and `min-width: 0`
   * (permission to fall to its longest word) next to a `flex: none` sibling
   * that never gives room. It had been patched at the text end more than
   * once. It is not patched now - the bespoke panel is gone and those
   * sections are ordinary sub-sections, so the column cannot come back. */
  note(
    !/settings-rule-panel|settings-rule-copy|settings-rules-grid|settings-rule-stack/.test(additions) &&
      !/settings-rule-panel|settings-rule-copy/.test(controller),
    'no settings section builds its own bordered panel with a prose column that can shrink to one word'
  );
  note(
    /actions \? el\('span', \{ class: 'sec-subhead-actions' \}, actions\) : null/.test(header),
    'a sub-section’s controls go in the heading’s own right-hand slot'
  );

  // The blue ⓘ beside a settings sub-heading used to be decoration with a
  // grey paragraph under it. One factory, and the paragraph lives behind it.
  note(/export function subhead\(el, \{ title, note = null, explain = null/.test(header), 'subhead is the one way a sub-heading is built');
  note(/\.sec-subhead-title \{/.test(controls) && /\.sec-subhead:has\(> \.sec-subhead-note\)/.test(controls), 'it has a title half and a note half, styled once');
  note(
    !/class: 'sec-subhead' \}, icon\(iconInfo\(\)\)/.test(controller) &&
      !/class: 'sec-subhead' \}, icon\(iconInfo\(\)\)/.test(cards),
    'no sub-heading still hand-builds a decorative ⓘ that opens nothing'
  );
  note(
    (controller.match(/subhead\(el, \{/g) || []).length === 7 && (cards.match(/subhead\(el, \{/g) || []).length === 1,
    'every sub-section goes through it'
  );
  const shared = readFileSync(join(ROOT, 'application', 'core', 'shared-helpers.js'), 'utf8');
  const print = readFileSync(join(ROOT, 'application', 'analysis', 'reporting-print.js'), 'utf8');
  note(
    /export const RECONCILE_MEANS =/.test(shared) && /\$\{RECONCILE_MEANS\}/.test(print),
    'and "reconciles" is defined in one place, which the printed report reads too'
  );
}

console.log('\n -- a jump leaves a way back, and a tab press ends it --');
{
  /* Every door Principle 8 opened moves the tab, the period and the filters
   * at once. Without a way back, following a sentence to its evidence cost a
   * person their place. These are the three things that keep the affordance
   * honest rather than permanent clutter. */
  note(
    /const drillToTransactions = jump\(drillToTransactionsRaw\);/.test(controller) &&
      /const switchLedgerView = jump\(switchLedgerViewRaw\);/.test(controller) &&
      /const openMonth = jump\(openMonthRaw\);/.test(controller) &&
      /const drillToAccountsPayee = jump\(drillToAccountsPayeeRaw\);/.test(controller) &&
      /const drillToRule = jump\(drillToRuleRaw\);/.test(controller) &&
      /const openStatementNudge = jump\(openStatementNudgeRaw\);/.test(controller) &&
      /const openEvidence = jump\(openEvidenceRaw\);/.test(controller),
    'every move that takes a person somewhere else is wrapped once, where it is defined'
  );
  note(
    /if \(_jumping\) return run\(\.\.\.args\);/.test(controller),
    'a move built out of other moves records ONE return point, not one per leg'
  );
  note(
    /const moved = placeKey\(from\) !== placeKey\(to\);/.test(controller) &&
      /const scrolled = opts\.returnOnScroll/.test(controller) &&
      /if \(moved \|\| scrolled\) _returnMark = from;/.test(controller),
    'and a move that changes nothing leaves no way back to offer'
  );
  note(
    /card: CARD_FACETS\.map/.test(controller) && /bank: BANK_FACETS\.map/.test(controller),
    'the place is read through the same facet registries that define what a filter IS'
  );
  note(
    /switchLedgerViewRaw\(id\);\s*\n\s*clearReturn\(\);/.test(controller),
    'pressing a tab ends any way back rather than recording one'
  );
  note(
    /const NAVIGABLE_VIEWS = new Set\(Object\.keys\(VIEW_LABELS\)\);/.test(controller) &&
      /if \(!NAVIGABLE_VIEWS\.has\(id\)\) return false;/.test(controller),
    'a bad view identifier cannot silently send a button somewhere else'
  );
  note(
    /function openStatementNudgeRaw\(\) \{\s*return switchLedgerViewRaw\('ahead', \{ anchorId: '#plan-statement-nudge' \}\);\s*\}/.test(controller) &&
      !/switchLedgerView\('plan'/.test(controller),
    'the next-statement action uses the Plan view’s real identifier and its named destination'
  );
  note(
    /btn\.classList\.toggle\('show', window\.scrollY > 80\);/.test(controller),
    'Back to top appears after an ordinary scroll on every view, not only a long transaction list'
  );
  const positionRenderer = readFileSync(join(ROOT, 'application', 'ui', 'position-render.js'), 'utf8');
  const accountsRenderer = readFileSync(join(ROOT, 'application', 'ui', 'accounts-render.js'), 'utf8');
  const dashboard = readFileSync(join(ROOT, 'interface', 'dashboard.css'), 'utf8');
  note(
    /explain: \[summary\.disclaimer, 'The copied version also includes dates, sources, and coverage\.'\]\.filter\(Boolean\)\.join\(' '\)/.test(positionRenderer),
    'the financial-summary explanation lives beside its title rather than below Copy summary'
  );
  note(
    /grid-template-columns: repeat\(auto-fit, minmax\(min\(100%, 19rem\), 1fr\)\)/.test(dashboard) &&
      /white-space: nowrap;/.test(dashboard.slice(dashboard.indexOf('.position-summary-hero-value'), dashboard.indexOf('.position-summary-grid'))),
    'position summary panels only take columns wide enough for their figures and the mobile headline stays on one line'
  );
  note(
    /summary: waiting \? `\$\{waiting\} to address` : 'All addressed'/.test(accountsRenderer) &&
      /alwaysOpen: waiting > 0/.test(accountsRenderer) &&
      /'Manage answers'/.test(accountsRenderer),
    'open review work is immediate, while settled review work is explicitly manageable'
  );
  note(
    !/@media \(max-width: 639px\) \{\s*\.to-top \{\s*display: none;\s*\}\s*\}/.test(additions),
    'the mobile back-to-top control is not disabled by a narrow-screen override'
  );
  const html = readFileSync(join(ROOT, 'index.html'), 'utf8');
  note(
    /id="go-back" class="to-top to-back"/.test(html),
    'the control is the back-to-top button\u2019s twin, not a second floating language'
  );
}
