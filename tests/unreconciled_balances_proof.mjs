...
note(bu.statementsPhrase(story.from, story.to, TODAY) === 'your Jun-26 statement', 'the span is named after the statement it starts from');
...
console.log('\n -- a typed balance does not outlive the statement it was for --');
{
  const updates = [
    { id: 'bank-gone', ledger: 'bank', account: '000111222', balance: 5000 },
    { id: 'bank-stays', ledger: 'bank', account: '000333444', balance: 7000 },
    { id: 'card-gone', ledger: 'card', account: '4321', balance: 900 },
  ];
  const removed = bu.pruneBalanceUpdates(updates, {
    bankAccounts: new Set(['000333444']),
    cardStatementsRemain: false,
  });
  note(removed.length === 1 && removed[0].id === 'bank-stays', 'a typed balance for an account with no statement left is dropped, a survivor kept');
  const kept = bu.pruneBalanceUpdates(updates, {
    bankAccounts: new Set(['000111222', '000333444']),
    cardStatementsRemain: true,
  });
  note(kept.length === 3, 'nothing is dropped while every account still has a statement');
  note(bu.pruneBalanceUpdates(null, { bankAccounts: new Set(), cardStatementsRemain: false }).length === 0, 'a missing list is treated as empty, not a crash');
}
