...
import { coverageSummary, coverageTimeline } from '../application/analysis/coverage-map.js';
import { formatMonthYear, monthIndex } from '../application/core/shared-helpers.js';
...
note(formatMonthYear('2026-09') === 'Sep-26', 'month-only dates use one compact app-wide format');
const investmentCoverage = coverageTimeline({
  investmentMonths: ['2026-01', '2026-02'],
  ledgers: ['investment'],
});
note(
  coverageSummary(investmentCoverage, 'investment').loaded === 2,
  'investment statements draw their own coverage bar without inventing another ledger'
);
...
note(/scheduled payments are covered/.test(exact.verdict.text), 'an exact fit is described as covered');
