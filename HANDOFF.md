...
**One confirmed, real, NOT-fixed defect — the info popover can render completely off-screen.** Isolated repro (not a fluke; reproduced twice, with 300ms and 1500ms waits, identical result): on the Activity tab, with several cards opened and the page scrolled ~2750px down, tapping/clicking a `.pfa-info-trigger` that is genuinely on-screen (scrolled into view, confirmed by its own `getBoundingClientRect()` immediately before the click) opens the popover (`present: true`, correct content text) but positions it at `transform: translate(12px, -816px)` on a `position: fixed` wrapper — 816px above the top of the viewport, completely invisible.

**Follow-up session (this pass): both original theories investigated further; neither confirmed, one real secondary defect found and reverted (not committed).**
- **Ruled out: floating-ui timing.** Already ruled out in the prior pass (300ms vs 1500ms wait gave identical results) — re-confirmed, not re-litigated.
- **Ruled out: a CSS `transform`/`filter`/`perspective`/`contain`/`will-change` ancestor creating a bad containing block for `position: fixed`.** Walked the full ancestor chain of the actual trigger via `getComputedStyle` at the moment of the bug — the result was an empty list. No such ancestor exists. This was a live, direct check, not an inference.
- **New finding, confirmed real but insufficient on its own: `PfaInfoPopover` never wraps `Popover.Content` in Radix's `<PopoverPrimitive.Portal>`.** Confirmed via DOM inspection at the moment of the bug: the popper wrapper was 13 DOM levels deep inside `.chart-info` (`isBodyChild: false`), not portaled to `document.body` as Radix's own documented best practice calls for. This is a genuine, independent correctness gap — not the exact cause of the -816 number, but a real deviation from correct Radix usage that's worth fixing on its own merits.
- **Fix attempted, tested, and found insufficient**: added `<PopoverPrimitive.Portal>` around `Popover.Content` in `react-ui/components/pfa-info-popover.jsx`, rebuilt, and re-ran the exact documented repro. Result: the popover now correctly portals to `document.body` (`isBodyChild: true`, `depthFromBody: 0`) — but the position is *still* wrong (observed `top: 3075` against a trigger at viewport `y: 490`, and separately `top: 1759`, both times far outside the viewport, still not matching the trigger's real position). Since the fix did not resolve the reported symptom, per this project's explicit instruction not to commit unverified work, **this change was reverted** (`git checkout -- react-ui/components/pfa-info-popover.jsx application/ui/react-dist/pfa-react.js`) rather than committed. The working tree is clean; nothing from this attempt is on disk or in git history.
- **Toast-interference theory: still not distinguished.** Ran out of budget before testing this directly (e.g. dismissing `mountTopGreeting`'s toast explicitly before the repro, or delaying the repro past its 6000ms auto-dismiss + 320ms removal animation, and comparing). This remains a live, untested theory.
- **A genuinely realistic, non-synthetic repro was not found.** A single ordinary interaction (open one card near the top of the page after a modest scroll, tap its icon) did not reproduce the bug in the time available — in fact, under a normal single-card-open + gradual-scroll scenario, no popover trigger was even reachable without first deep-scrolling and opening several cards, meaning the practical exposure of this bug to a real user doing normal things is still unclear. **This is the top item for whoever picks this up next**: with the Portal fix now known to be necessary-but-insufficient, the remaining work is to find why the *position value itself* is wrong even when correctly portaled — worth checking whether `triggerRef` (used elsewhere in the component for `.focus()` calls) is somehow being read by Radix instead of its own internal anchor ref, whether the forced-opening of many `<details>`/accordion elements in the test's own setup causes a layout thrash that floating-ui's one-shot position computation loses track of, and — still untested — the toast-interference theory above.
...
## Bug 2 (not reached to a fix) — "Review & adjustments" card layout, reported misaligned vs siblings

Reported: on Activity → Analysis, the "Review & adjustments" card's expanded rows (date/description, amount, "Is this income? Yes/No") don't follow the label-left/summary-right pattern its sibling cards use. Investigated via code reading; **could not be visually reproduced or confirmed in this pass**, for a concrete, stated reason — not abandoned for lack of trying.

**What was traced (static, confirmed by reading the real files):**
- The card itself: `application/ui/accounts-render.js` (~line 445), already React — built via `collapsibleCardReact(el, { title: 'Review & adjustments', ... })`, the same shared factory every sibling card in the list uses. So the card's own header row (icon, title, summary, chevron) is structurally identical to its siblings by construction — if there's a visible header-row misalignment, it isn't from a different code path.
- The expanded body content is vanilla DOM (not React), built in `renderLedgerReview`/`reviewRow` in the same file. Each row is `.review-row` containing a `.recurring-row` (the same shared grid class — `display: grid; grid-template-columns: 1fr auto auto;` — used by other row lists like "Biggest payments") for the date/description/amount line, plus a `.confirm-line` (from `application/ui/confirm-control.js`'s `line()` helper) for the Yes/No question underneath.
- `.confirm-line`'s CSS (`interface/feature-additions.css` ~line 4340) is itself a deliberately-fixed shared pattern with an explicit comment explaining why it uses `margin-left: auto` on the control rather than `justify-content: space-between` (the latter caused a label-left/control-right row to flip sides depending on label length when it wrapped) — this reads as the *correct*, already-reused mechanism, not a one-off in need of fixing.
- Nothing read in this pass points to an obvious, confirmable defect in this code. It's possible the real bug is in something not yet read (e.g. `.review-adjustments-body`'s own container styling, or a CSS rule that only matches the old native-`<details>` DOM shape and silently no-ops under this card's current structure — the exact class of bug this migration has hit before, per section 2's `chartInfoReact` positioning issue above) — but this wasn't confirmed.

**Why it wasn't visually verified**: the only available sample persona ("Trevor") has zero pending review items in its current state (`waiting = 0`), so the card renders collapsed with "All addressed" and none of the reported rows (date/description/amount/Yes-No) ever render to inspect. Clearing the `confirmations` IndexedDB store directly (to force `waiting > 0`) did not produce pending rows either — meaning `waiting` depends on the persona's underlying transaction data actually containing `cashDeposit`/`refundLike`-flagged rows, which Trevor's dataset may simply not have any of, not on confirmation state alone. No screenshot of the reported bug was available in this session's context to compare against (the task described one as "attached/referenced," but no image was actually receivable here).

**What remains to be done**: find or construct sample data with at least one pending cash-deposit or refund row (or add a throwaway one via the mock/dev tooling) so the card actually renders its expanded content, screenshot it next to a sibling row list (e.g. "Biggest payments"), and only then decide whether there's a real CSS/structural defect to fix — per this task's own instruction, do not guess a fix without seeing the actual misalignment. No code was changed for Bug 2 in this pass.
