...
export function allocationView(allocation, money, prose = money) {
  ...
  if (a.nothingSpare) {
    return {
      ...
      tone: 'watch',
      detail:
        'Fixed expenses due before your next pay account for everything on hand, so nothing is free to spend yet.',
      earmarkText: money(0),
  ...
  return {
    ...
    detail: `Your plan allocates ${a.sharePct}% of a normal month to free spending. Applied to the ${prose(a.surplus)} left after fixed expenses, that is ${prose(a.spendable)}. The other ${prose(a.earmarkedSaving)} is held for saving${a.usingDefault ? ', using the starting 60/20/20 split until you set your own' : ''}.`,
