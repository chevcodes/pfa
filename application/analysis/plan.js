...
export function planGroups(cfg) {
  ...
  return Array.isArray(g) && g.length
    ...
    : [
        ...
        { key: 'free', label: 'Free spending', meaning: '' },
      ];
}
...
// category nobody has placed falls to free spending, which is the only group
 ...
 *
 * Free spending being UNDER its target is not a problem to surface - the plan
...
export function buildPlan({
  ...
  const groups = planGroups(cfg).map((g) => {
    ...
    return {
      ...
      targetAmount: t,
      diff,
      // Free spending is the one group where being UNDER target is not a
...
function commitmentsMoved(trend, month, fixedMonthly, tolerance = 0.15) {
  ...
  return {
    known: true,
    usual: !short,
    text: short ? 'not everything has left the account yet' : 'everything moved as usual',
  };
}
...
function cardSentence(card) {
  ...
  if (card.previousOwed != null && Number(card.previousOwed) > 0) {
    ...
    if (owed > prev) return 'the card is carrying a balance that grew';
    if (owed < prev) return 'the card is carrying a balance, smaller than last month';
    return 'the card is carrying a balance, unchanged';
  }
  return 'the card is carrying a balance';
}
...
export function buildPlanModel(plan, cfg = {}) {
  ...
  // band and again in the card's closed summary ("Free spending: $83,961.14
  ...
  return {
    ...
    //
    // This used to sort by size, so the hero listed "Free spending, Fixed
    ...
    free: {
      ...
      label:
        plan.confidence === 'incomplete'
          ? 'estimated free spending allocation'
          : 'free spending allocation',
