...
import { coverageBankSpan } from './reporting-periods.js';
...
// A typed balance for an account with no statement left to reconcile it
// against must not be carried forever, invisibly - it would never be
// applied or superseded again (resolveBalances only ever acts on an account
// present in `known`, rebuilt fresh from current statements), so it would
// just sit in storage, and in every future encrypted backup, with no trace
// in the UI. Called from the statement-removal and same-month-collision
// cascades (manage-data.js, app-intake.js) - never from statement-cascade.js
// itself, which is a "history" calculation source and must never read or
// write an entered balance.
export function pruneBalanceUpdates(balanceUpdates, { bankAccounts, cardStatementsRemain }) {
  const survivingBankIds = new Set([...(bankAccounts || [])].map(accountId));
  return (balanceUpdates || []).filter((update) => {
    if (!update) return false;
    if (update.ledger === 'bank') return survivingBankIds.has(accountId(update.account));
    if (update.ledger === 'card') return !!cardStatementsRemain;
    return true;
  });
}
...
export function statementMonthLabel(iso, today) {
  return formatMonthYear(String(iso).slice(0, 7));
}
