...
export function buildGoalModel(goal, progress, guard, cfg = {}, opts = {}) {
  ...
  if (progress.type === 'cushion') {
    ...
    const period = progress.month ? ` for ${formatMonthYear(progress.month)}` : ' this period';
    detail =
      progress.remaining >= 0
        ? `${money(progress.spent)} of your ${money(progress.ceiling)} limit${period}, ${money(progress.remaining)} left.`
        : `${money(progress.spent)} against your ${money(progress.ceiling)} limit${period} - ${money(Math.abs(progress.remaining))} over.`;
  } else {
    lead = '-';
    tag = '';
    tone = 'neutral';
