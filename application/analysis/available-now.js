...
function buildVerdict(p, cardV, incomeV) {
  ...
  if (!incomplete) {
    parts.push(
      covered
        ? 'scheduled payments are covered before your next pay'
        : 'scheduled payments exceed the amount available before your next pay'
    );
  }
...
export function buildAvailableNowModel(primitiveResult, cfg = {}, planTargets = null) {
  ...
  const hasPaymentsBeforePayday = p.layers.commitmentsBeforeIncome > 0;

  /* A GUESS, NAMED, where it does its damage.
   *
   * Which repeating credit is "your pay" sets the next payday, and the payday
   * sets the window of payments counted here - so this figure, and the lead
   * figure it feeds, both turn on it. Where more than one credit repeats and
   * nobody has said which is the pay, the app takes the largest and used to say
   * nothing, which made a choice look like a fact. Silent only when there was
   * no choice to make. */
  const payBasis = !p.income
    ? ''
    : p.income.chosenBy === 'person'
      ? ' Your payday comes from the payee you named.'
      : p.income.otherCandidates > 0
        ? ` Your payday is taken from the largest of ${p.income.otherCandidates + 1} payments that repeat like this. If that is the wrong one, say so on any of its transactions.`
        : '';
  ...
  const lead = {
    ...
    detail:
      allocated.detail +
      ' Fixed-expense categories set your normal-month plan; this forecast deducts only repeating or card payments due before payday.',
  };
  ...
  const leadNote = allocation.nothingSpare
    ? {
        text: `Payments due before payday account for the ${prose(p.layers.availableBalance)} on hand.`,
        tone: 'watch',
      }
    : {
        text: `${allocated.earmarkProse} of the ${prose(allocation.surplus)} available before payday is held for saving.`,
        tone: 'neutral',
      };
  ...
  const working = [
    {
      ...
      tag: hasPaymentsBeforePayday ? 'everyday accounts' : 'no scheduled payments before payday',
      tone: 'neutral',
      detail:
        ...
        (hasPaymentsBeforePayday
          ? ''
          : ' Fixed-expense categories set the normal-month plan; only a repeating or card payment due before payday reduces this forecast.' +
            payBasis),
    },
  ];

  if (hasPaymentsBeforePayday) {
    working.push(
      {
        id: 'commitments',
        label: 'Payments due before payday',
        amount: p.layers.commitmentsBeforeIncome,
        amountText: money(p.layers.commitmentsBeforeIncome),
        tag: `${nCommit} payment${nCommit > 1 ? 's' : ''}${hasCardCommit ? ' incl. card' : ''}`,
        tone: 'neutral',
        detail:
          `Payments due to leave before your next pay: ${p.commitments.map((c) => `${money(c.amount)}${c.basis === 'card' ? ' (card)' : ''} on ${dateText(c.date)}`).join('; ')}.` +
          payBasis,
      },
      {
        id: 'surplusAfterCommitments',
        label: 'Available before payday',
        amount: p.layers.estimatedAvailableAfterCommitments,
        amountText: money(p.layers.estimatedAvailableAfterCommitments),
        tag: 'after scheduled payments',
        tone: 'neutral',
        detail: `Cash on hand of ${prose(p.layers.availableBalance)} minus ${prose(p.layers.commitmentsBeforeIncome)} of scheduled payments. Your plan divides this figure; it is not itself what is free to spend.`,
      },
    );
  }

  if (allocation.earmarkedSaving > 0) {
    working.push({
      id: 'earmarkedSaving',
      ...
      tone: 'neutral',
      detail:
        'Money your plan reserves for saving after discretionary spending. It counts as savings once you move it.',
    });
  }
