import { formatDisplayDate, joinWithAnd, monthsBetween, roundMoney as r2 } from '../core/shared-helpers.js';
import {
  coverageBankSpan,
  monthName,
  detectRecurring,
  ...
  for (const item of ctx.commitments.combined.items || []) {
    ...
    if (!then) {
      ...
      out.push({
        ...
        detail: `First seen in ${monthName(seen)}.`,
      });
      ...
      out.push({
        ...
        detail: `Since ${monthName(item.risen.sinceMonth)}.`,
      });
    }
  }
  return out;
...
function investmentCauses(ctx) {
  ...
  for (const source of sources) {
    ...
    result.causes.push({
      ...
      detail: '',
      link: { kind: 'view', view: 'position', anchorId: '#position-investments' },
    });
  }
  return result;
}
  ...
  return {
    ...
    stateLine:
      !story && baseline.frontier && !baseline.unreconciled
        ? `Your statements through ${monthName(baseline.frontier.slice(0, 7))} all add up.`
        : null,
  };
}
