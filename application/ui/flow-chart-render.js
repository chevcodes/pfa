...
import { monthTickOf, chartIsHidden, renderHiddenChart } from './chart-helpers.js';
...
export function createFlowChartRenderer(ctx) {
  requireCtx(ctx, ['el', 'bankMoney', 'monthLabel', 'monthShort', 'openMonth'], 'createFlowChartRenderer');
  ...
  function renderFlowChart(trend, opts = {}) {
    ...
    if (chartIsHidden()) return renderHiddenChart(el, 'Cash movement', { height: '220px' });
    const root = markProportional(el('div', { class: 'fl-chart', role: 'group', 'aria-label': 'Cash movement' }));
    ...
    for (const [key, label, tone] of [['income', 'Cash inflow', 'in'], ['spending', 'Cash outflow', 'out']]) {
      ...
      // The year appears on the ticks whenever these rows span more than one -
      // see monthTickOf. Reading a month off the axis must not need a hover.
      const chart = renderColumnChart({ ...ctx, monthShort: monthTickOf(MONTHS_SHORT, rows) }, {
        label,
        rows,
        money: bankMoney,
        min: 0,
        ...
        hideLegend: true,
        onInspect: inspect,
        // Twenty-four buttons, each already naming its month and both of that
        // month's figures, and none of them went anywhere. This is the one
        // chart on the screen a person lands on.
        onSelect: (row, event) => {
