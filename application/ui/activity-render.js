...
export function createActivityRenderer(ctx) {
  ...
  function renderCommittedFlexible() {
    ...
    const lead = m.lead || {};
    const support = [m.committed, m.flexibleSpent].filter(Boolean).map((c) => ({
      text: c.amountText,
      label: c.label,
      tag: c.tag,
      tone: c.tone,
  ...
  *    "Custom label \u00d7N" control that opens the same label picker this card's
    ...
    return el(
      ...
      el(
        ...
        chartInfo(
          el,
          '',
          'Group spending that belongs together but spans categories and months - a renovation, a holiday, a trip.'
        )
      )
    );
  }
  ...
  function openTransactionsForSearch(term) {
  ...
  function matchesCategories(m) {
    ...
    return _txCategories.has(m.row.category);
  }
    ...
    const renderMergedRow = (m) => {
      ...
      const nameCell = el(
        ...
        el(
          'div',
          { class: 'muted small', style: 'margin-top:2px' },
          // The SAME category badge a card row gets. Bank rows are categorised
          // by the same categorise() door (classifiedBank), but this meta line
          // never showed the result - so a correctly categorised bank
          // transaction looked uncategorised beside a card one, and the two
          // ledgers read as two different kinds of row.
          // A bank category is DERIVED, not chosen: setCategory writes only to
          // state.records (the card ledger), and a bank row's category comes
          // from the rules engine at render time. So the tag stays a plain span
          // here - and now says why, instead of leaving a person to discover by
          // tapping that this one alone does not open.
          catTag(r.category, {}),
          chartInfo(
            el,
            '',
            'Bank categories are set automatically from your rules, not chosen row by row. To change how transactions like this are filed, edit the rule under Data & settings, Rules and categories.'
          ),
  ...
  function renderPace() {
    ...
    function goSetCeiling(category) {
      ...
      const ceilings = document.getElementById('activity-ceilings');
      const details = ceilings ? ceilings.querySelector('details') : null;
    ...
    const highest = pace[0];
    const ratio = highest.typical > 0 ? Math.round((highest.projected / highest.typical) * 100) : null;
    return collapsibleCard(el, {
      title: 'Partway through the month',
      icon: icon(iconGap()),
      summary: `${highest.category}: about ${money0(highest.projected)} projected${ratio ? ` · ${ratio}% of typical` : ''}`,
      body: sec,
      name: 'activity-current-month-pace',
    });
  }
  ...
  function renderTransactionsTab() {
    ...
    const edge = balanceUpdates.reconciledEdge(selected, balanceContext);
    if (edge) wrap.append(edge);
    ...
    const hint = el('div', { class: 'tx-hint tx-cats-label' });
    hint.append(
      chartInfo(el, 'Categories in use', [
        el(
          'p',
          { style: 'margin:0 0 8px' },
        ...
        el(
          'p',
          { style: 'margin:0' },
          'Card categories can be changed: tap the category under a card transaction. Bank categories are set automatically from your rules and are not edited row by row - change them under Data & settings, Rules and categories.'
        ),
      ])
    );
    ...
    if (usedCategories.length) {
      ...
      categoryFilter.append(hint, chips);
      markScrollAffordance(chips);
    } else {
      hint.append(el('span', { style: 'margin-left:8px;font-weight:400' }, 'none yet.'));
      categoryFilter.append(hint);
    }
    wrap.append(categoryFilter, searchFilters);
