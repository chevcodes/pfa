  ...
  function renderIncomeChart(income, opts = {}) {
    ...
    function build(mode) {
      return renderColumnChart({ ...ctx, monthShort: shortMonthOf(MONTHS_SHORT) }, {
        ...
        // Names WHOSE typical this is - analyseIncomePattern's primary income
        // source - so it is not mistaken for the whole month's income.
        guideLabel: model.label ? `Typical ${model.label}` : 'Typical payment',
        mode,
        // A bar here is ONE payee's deposit in one month, and the card
        // already has the door to that payee - the figure above the chart.
        // The bar adds the month, so it opens the deposit it is drawn from
        // rather than announcing it and staying put.
        ...(typeof opts.onMonth === 'function' ? { onSelect: (row) => opts.onMonth(row.month) } : {}),
