...
export function createAheadRenderer(ctx) {
    ...
    if (reason === 'gap') {
      lines.append(
        chartInfo(
          el,
          'A month is missing',
          `A projection assumes one month follows the next. ${gapMonths.map((m) => monthLabel(m)).join(', ')} has no bank statement, so the run is broken.`
        )
      );
  ...
  function renderBoundaryForm() {
    ...
    const options = [
      ...
      { kind: 'calculated', label: 'Fixed expenses plus a cushion of days' },
