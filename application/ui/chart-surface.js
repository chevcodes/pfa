...
import { chartIsHidden, renderHiddenChart, proportionShares } from './chart-helpers.js';
import { growIn, staggerIn, drawPath } from './motion.js';
import { formatMonthYear, markScrollAffordance } from '../core/shared-helpers.js';
...
export function renderColumnChart(ctx, spec) {
  ...
  // 568px of chart in a 313px card on a phone. Same treatment as the chip
  // strips: the fade marks the side that has more, so a chart that runs off the
  // edge reads as scrollable rather than cut off.
  const scroll = markScrollAffordance(el('div', { class: 'chart-scroll' }));
  /* A LONG SERIES IS A SHAPE, NOT A ROW OF COLUMNS.
   *
   * 42px a month is the width a bar needs to be tappable and readable. Held
   * to on a forty-four month value line it made a canvas nearly two metres of
   * pixels wide inside a 740px card, so the only part ever on screen was a
   * flat eighteen-month thread while the peak that set the scale sat off the
   * right edge - the chart looked broken because the shape was never visible
   * at once. Past eighteen points the columns narrow enough for the whole run
   * to fit, and the month labels thin to the year boundaries so they stop
   * colliding. */
  const dense = rows.length > 18;
  const columnStep = dense ? 14 : 42;
  const canvas = el('div', { class: 'chart-canvas', style: `min-width:${Math.max(260, rows.length * columnStep + 64)}px` });
  ...
  for (const s of series) {
    ...
    gradient.append(
      chartSvg('stop', {
        offset: '100%',
        'stop-color': ink,
        'stop-opacity': 'var(--chart-gradient-tail)',
      })
    );
  ...
  /* POINTS ARE ROUND.
   *
   * The plot stretches its viewBox to the canvas (preserveAspectRatio:none),
   * which is right for bars - they are measured in one direction - and wrong
   * for a circle: r="4" came out roughly half as tall as it was wide, so a
   * line chart was strung with grey lozenges. Non-scaling-stroke fixes the
   * OUTLINE of a shape under that transform, never its geometry.
   *
   * So the points leave the stretched coordinate system and join the layer
   * that is already positioned in percentages over it - the same one the hit
   * targets use - where a circle is a circle at any width. */
  const dotLayer = el('div', { class: 'chart-dots', 'aria-hidden': 'true' });
  ...
  rows.forEach((row, index) => {
    ...
    for (let j = 0; j < series.length; j++) {
      ...
      if (spec.mode === 'line') {
        // No dot here. Every point is drawn ONCE, below, from `points` - this
        // appended a second circle on top of that one, so every marker on
        // every line chart was two overlapping discs with two outlines.
        if (row.marker) {
          svg.append(chartSvg('line', { x1: cx, x2: cx, y1: Math.max(0, y(plotted) - 18), y2: Math.min(240, y(plotted) + 18), class: 'chart-marker' }));
        }
    ...
    const name = formatMonthYear(row.month);
    const detail = [name, ...series.map((s) => `${s.label}: ${row.present === false || row[s.key] == null ? spec.missingText || 'No deposit found' : money(row[s.key])}`)];
    ...
    if (spec.onSelect) target.addEventListener('click', (event) => { if (!privateViewOn()) spec.onSelect(row, event); });
    target.addEventListener('keydown', (event) => {
      let next = null;
    ...
    const short = tick ? (tick.month ? tick.month(row.month) : tick(row.month)) : name;
    const year = tick && tick.year ? tick.year(row.month) : '';
    // On a dense run only the year boundaries and the two ends are named. The
    // slot is still drawn so every label stays under its own point.
    const named = !dense || !!year || index === 0 || index === rows.length - 1;
    months.append(
      el(
        'span',
        { class: row.present === false ? 'is-missing' : '', title: name },
        named ? short : '',
        named && year ? el('span', { class: 'chart-month-year' }, year) : null
      )
    );
  ...
  if (spec.guide != null && spec.guide >= bottom && spec.guide <= top) {
    ...
    // The guide's own name, because "Typical" alone is ambiguous the moment two
    // charts on one screen mean different things by it. The income chart plots
    // ONE income source month by month, so its guide is that source's typical
    // payment - not the typical month's total income the Position and Plan tabs
    // report, which is a different figure and was being read as the same one.
    const guideLabel = spec.guideLabel || 'Typical';
    plot.append(el('span', { class: 'chart-guide', style: `top:${guideY / 240 * 100}%` }, `${guideLabel} ${axisMoney(spec.guide)}`));
  }
  plot.append(svg, dotLayer, hitLayer);
  ...
  if (rows.some((row) => row.present === false)) legend.append(el('span', {}, spec.missingLegend || '- No deposit'));
  if (spec.markerLabel && rows.some((row) => row.marker)) {
    legend.append(el('span', {}, el('i', { class: 'chart-key is-marker', 'aria-hidden': 'true' }), spec.markerLabel));
  }
  if (rows.some((row) => row.incomplete)) legend.append(el('span', {}, '⋯ Partial month'));
  // A caveat about what a stretch of the line MEANS is a key to the line, not
  // a paragraph under it: it belongs with the other keys, at their weight.
  if (spec.legendNote) legend.append(spec.legendNote);
...
export function renderProportionBar(ctx, spec) {
  const { el } = ctx;
  if (chartIsHidden()) return renderHiddenChart(el, spec.label, { height: '120px' });
  const money = spec.money || ctx.money0 || makeMoneyShort();
  const { bands, total, shareOf } = proportionShares(spec.bands);
  if (!bands.length || total <= 0) return null;

  const root = markProportional(
    el('div', { class: 'proportion', role: 'group', 'aria-label': spec.label })
  );
  const tips = chartTooltip(el, root);
  const track = el('div', { class: 'proportion-track' });
  const grown = [];
  for (const b of bands) {
    const share = Number(b.amount) / total;
    const detail = [b.label, money(b.amount), `${shareOf(b)}% of ${money(total)}`];
    if (b.meaning) detail.push(b.meaning);
    const seg = el('div', {
      class: `proportion-seg is-${b.key || 'neutral'}`,
      // Marked so the stylesheet can keep a zero band out of the bar while the
      // legend still lists it.
      'data-zero': Number(b.amount) > 0 ? null : '1',
      style: `flex-grow:${share}` + (b.colour ? `;background:${b.colour}` : ''),
      tabindex: '0',
      role: 'img',
      'aria-label': detail.join('. '),
    });
    tips.bind(seg, detail);
    track.append(seg);
    grown.push(seg);
  }
  root.append(track);

  const legend = el('div', { class: 'proportion-legend' });
  for (const b of bands) {
    const share = shareOf(b);
    const rowLabel = [b.label, money(b.amount), `${share}%`, b.meaning].filter(Boolean).join('. ');
    legend.append(
      el(
        'div',
        { class: 'proportion-legend-row', title: b.meaning || '', 'aria-label': rowLabel },
        el('i', { class: `proportion-key is-${b.key || 'neutral'}`, 'aria-hidden': 'true' }),
        // The meaning is carried by the row's own tooltip and accessible label
        // (bound above), not printed under every band. Three bands each with a
        // second line of prose tripled the legend's height and repeated what
        // the band names already say - the hero's own meaning line says
        // "yours to enjoy, guilt-free" directly above the legend that repeated
        // it for Free spending.
        el('span', { class: 'proportion-legend-label' }, b.label),
        el('span', { class: 'proportion-legend-amt num money' }, money(b.amount)),
        el('span', { class: 'proportion-legend-share' }, `${share}%`)
      )
    );
  }
  root.append(legend);

  staggerIn(grown, () => [{ transform: 'scaleX(0)' }, { transform: 'scaleX(1)' }], {
    duration: 460,
    step: 90,
  });
  return root;
