...
export function createPlanRenderer(ctx) {
  ...
  const coverage = createCoverageStrip({ el, monthLabel: ctx.monthLabel || ((m) => m) });
  ...
  function renderCoverage(opts = {}) {
    ...
    const timeline = coverageTimeline({ cardMonths, bankMonths, coverage: state.coverage });
  ...
  function planModel(overrideTargets) {
    ...
    const raw = buildPlan({
      ...
      designatedSetAside: state._planSetAside || null,
  ...
  function renderPlanHero() {
    ...
    const why = [
      ...
      el(
        ...
        workingRow('Card free spending', model.workings.cardFreeText)
      ),
  ...
  function renderPlanLever() {
    ...
    if (wizardOpen()) return wizard.renderWizard(questions);
    ...
    head.append(
      chartInfo(
        el,
        'How this works',
        'Each group takes a share of a normal month. A group is decided by whether the money is owed, not how often it goes out - dining every week is still free spending. 60/20/20 is a starting point, not a pass mark.'
      )
    );
    if (questions.length) {
    ...
    function repaint() {
      ...
      totalPill.textContent = balanced
        ? `${total}% of take-home`
        : `${total}% - tap to make it 100%`;
      totalPill.className = 'plan-total' + (balanced ? ' is-balanced' : ' is-off is-fixable');
      ...
      row.append(
        el(
          ...
          el(
            ...
            el('span', { class: 'plan-row-label' }, g.label)
          ),
    ...
    const balance = el('button', { class: 'btn', type: 'button' }, 'Make it 100%');
    balance.addEventListener('click', balanceShares);

    const reset = el('button', { class: 'btn', type: 'button' }, 'Reset to 60/20/20');
  ...
  function offTargetSummary(model) {
    ...
    return `${worst.label}: ${worst.trackText}`;
  }
  ...
  function renderSavingsPicker() {
    ...
    const hasChosen = Array.isArray(state._planSetAside);
    ...
    const chosen = new Set(hasChosen ? state._planSetAside : suggested);
    ...
    for (const d of dests) {
      ...
      box.addEventListener('change', async () => {
        ...
        const adding = !next.has(d.key);
        if (adding) next.add(d.key);
        else next.delete(d.key);
        _openDrawer = 'savings';
        _restoreFocusTo = box.id;
        await reversible.change({
          metaKey: 'planSetAside',
          stateKey: '_planSetAside',
          next: [...next],
          describe: () =>
            adding ? `${d.label} now counts as saving.` : `${d.label} no longer counts as saving.`,
          track: () => trackUsage && trackUsage('plan-designate-savings'),
        });
      });
      row.append(action);
      list.append(row);
  ...
  function renderAssignments() {
    ...
    for (const g of groups) {
      ...
      const block = el('div', { class: 'plan-assign-group' });
      block.append(
        el(
          'h5',
  ...
  function planDraftSignature() {
    ...
    const sa = (state._planSetAside || []).join(',');
