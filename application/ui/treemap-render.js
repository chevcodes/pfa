...
/* Two categories on screen together must never read as the same colour.
 *
 * This used to be decided on HUE ANGLE, with a separate branch for greys
 * (rotating a grey's hue changes nothing) - two proxies for the question
 * actually being asked, which is "would a person tell these two apart". Hue
 * angle is a poor proxy in both directions: a 24-degree gap between two deep
 * blues is invisible, while the same gap between a yellow and an orange is
 * obvious. Perceptual distance asks the question directly and needs no special
 * case for greys.
 *
 * 15 dE is about "clearly a different colour" - well past the ~2.3 threshold
 * where a difference becomes visible at all, and low enough that a picture of
 * ten categories can still be filled from a fourteen-entry palette. */
const MIN_TILE_DISTANCE = 15;

function tooClose(candidate, takenDrawn) {
  for (const other of takenDrawn) {
    if (colourDistance(candidate, other) < MIN_TILE_DISTANCE) return true;
  }
  return false;
}
...
/* LIGHT MODE, RESTATED IN L*, AND WHITE INK IN BOTH THEMES.
 *
 * The band this replaces was written in HSL lightness with the stated goal
 * that "every tile carries the same visual weight whatever its hue". Measured
 * on the running app it did the opposite: clamped to HSL L 0.56-0.66 the ten
 * tiles landed between L* 53.8 (a purple) and L* 79.8 (a yellow-green) - a
 * 26-point perceptual spread, drawn as pale washes on a white page with grey
 * type on them. HSL lightness is not weight; L* is.
 *
 * The light map now takes the same ink as the dark one: white. That single
 * decision sets the band, because white type is the binding constraint:
 *   - white must clear 4.5:1 on every tile -> L* <= 49.8;
 *   - and a tile must not read as black -> L* >= 42.
 * L* 42-49 satisfies both, separates hugely from the white page (4.6:1 to
 * 6.0:1 at the extremes), and holds every hue at one perceived weight. */
const LIGHT_LSTAR = { lo: 42, hi: 49 };
...
const LIGHT_SAT = { lo: 0.58, hi: 0.82 };
...
/* CIE L* from relative luminance. L* depends on Y alone, so this is exact
   rather than an approximation, and it is what the eye reads as "how light is
   this", unlike the L of HSL. */
function lStar(hex) {
  const y = relLuminance(hex);
  return y > 0.008856 ? 116 * Math.cbrt(y) - 16 : 903.3 * y;
}

/* CIE Lab, for measuring how far apart two fills actually look. Hue gaps are a
   cheap proxy and they are what tooClose screens on; this is the tie-break when
   no candidate passes that screen, so the picture still gets the most separated
   colour available rather than whichever one the search happened to stop on. */
function toLab(hex) {
  const h = String(hex).replace('#', '');
...
export function tileSurface(hex, light) {
  ...
  if (light) {
    // Walk HSL lightness until the PERCEIVED lightness lands in the band. Each
    // hue needs a different HSL lightness to arrive at the same L*, which is
    // exactly why clamping HSL lightness could not deliver an even picture.
    const sat = Math.min(LIGHT_SAT.hi, Math.max(LIGHT_SAT.lo, s));
    let lightness = Math.min(0.95, Math.max(0.05, l));
    let out = hslToHex(h, sat, lightness);
    for (let i = 0; i < 120; i++) {
      const L = lStar(out);
      if (L >= LIGHT_LSTAR.lo && L <= LIGHT_LSTAR.hi) break;
      const step = L > LIGHT_LSTAR.hi ? -0.01 : 0.01;
      const next = Math.min(0.95, Math.max(0.05, lightness + step));
      // Only equal when the clamp refused the step: the hue cannot be moved
      // any further and this is as close to the band as it gets.
      if (next === lightness) break;
      lightness = next;
      out = hslToHex(h, sat, lightness);
    }
    // The ink is only ever verified here, never optimised for: the band above
    // already clears it, and this is the guard for a hue that could not be
    // brought into the band at all. White is the ink, so a failure is darkened.
    for (let i = 0; i < 25 && contrastRatio(out, '#ffffff') < LIGHT_MIN_CONTRAST; i++) {
      lightness = Math.max(0.05, lightness - 0.02);
      out = hslToHex(h, sat, lightness);
    }
    return out;
  }
...
/* `surface` is the transform the tile will actually be painted with.
 *
 * Separation used to be decided on the RAW palette colour and the painted tile
 * was then pulled into a narrow lightness/saturation band - so two entries
 * that were far enough apart in the palette could land on top of each other
 * once drawn. Measured in light mode, Pharmacy & Health (#4e95d0) and
 * Insurance (#4e8ed0) came out 5.5 dE apart: one colour, two categories.
 * Testing the SURFACED colour puts the decision on the thing a person sees. */
export function resolveDistinctTileFills(names, catColour, surface = (c) => c) {
  const takenDrawn = [];
  ...
  for (const raw of names) {
    ...
    /* How far this candidate lands from everything already on the picture,
       measured on the SURFACED colours in Lab. */
    const separation = (c) => {
      const drawn = surface(c);
      let min = Infinity;
      for (const other of takenDrawn) min = Math.min(min, colourDistance(drawn, other));
      return min;
    };
    let candidate = base;
    let attempt = 0;
    let best = { colour: base, gap: separation(base) };
    while (tooClose(surface(candidate), takenDrawn) && attempt < 8) {
      attempt++;
      ...
      const gap = separation(candidate);
      if (gap > best.gap) best = { colour: candidate, gap };
    }
    /* The search gives up after eight tries, and it used to keep whatever the
       eighth try produced. shiftHue steps 47 degrees, so the eighth step is
       376 degrees - sixteen degrees from where it started. Insurance and
       Pharmacy & Health therefore came out 3.5 dE apart in light mode: the
       run ended exactly where a colour is LEAST separated. Keeping the best
       candidate seen instead makes exhausting the search harmless. */
    if (tooClose(surface(candidate), takenDrawn)) candidate = best.colour;

    takenDrawn.push(surface(candidate));
...
export function createTreemapRenderer(ctx) {
  ...
  function readableInk(hex) {
    ...
    const surface = '#' + h.slice(0, 6);
    return contrastRatio(surface, INK_DARK) >= contrastRatio(surface, '#ffffff')
      ? INK_DARK
      : '#ffffff';
  }
      ...
      if (valueFits) {
        ...
        const movement =
          comparison && (comparison.direction === 'up' || comparison.direction === 'down')
            ? ` ${arrow}`
            : '';
        // "23% ▲" reads as "up 23%". It is not: 23% is this category's SHARE of
        // the period, and the arrow is its direction against last period - two
        // different quantities run together into one string, with the actual
        // change (486%, say) living only in the tooltip. The share is now
        // labelled as a share wherever the tile has room for the word, and
        // falls back to the bare percentage on a tile too small for it. The
        // arrow keeps its own meaning either way.
        const sharePct = `${share.toFixed(share >= 10 ? 0 : 1)}%`;
        const shareLabelled = `${sharePct} of spend${movement}`;
        const shareBare = `${sharePct}${movement}`;
        const shareText = share >= 0.1 ? shareLabelled : null;
        const roomForLine =
          shareText && availH - linesH - valueFont - LINE_GAP >= shareFont + LINE_GAP;
        // Prefer the labelled form; drop to the bare percentage only when the
        // tile genuinely cannot hold the words.
        const chosenShare = !roomForLine
          ? null
          : estWidth(shareLabelled, shareFont) <= availW
            ? shareLabelled
            : estWidth(shareBare, shareFont) <= availW
              ? shareBare
              : null;
        const shareFits = !!chosenShare;
        if (shareFits) {
  ...
  function renderTreemapPicture(tiles, subTiles, onTileClick) {
    ...
    /* One shared depth overlay for every tile.
     *
     * The light map's fills are now as deep as the dark map's, so both take the
     * same restrained treatment: a whisper of highlight at the head and a
     * shade at the foot, over a solid fill. What it must NOT do is what it did
     * when the tiles were pale washes - lift the top 46% of every fill 16%
     * toward the white page it was meant to stand out from. */
    ...
    sheen.append(
      svgEl('stop', {
        offset: '0%',
        'stop-color': '#ffffff',
        'stop-opacity': lightSurface ? 0.07 : 0.16,
      })
    );
    ...
    sheen.append(
      svgEl('stop', {
        offset: '100%',
        'stop-color': '#000000',
        'stop-opacity': lightSurface ? 0.14 : 0.18,
      })
    );
    defs.append(sheen);
