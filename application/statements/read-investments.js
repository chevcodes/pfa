import { fnv1a, money, roundMoney, MONTHS, recordMissingRequiredField } from '../core/shared-helpers.js';
...
function parseScotiaInvestmentStatement(segLines, sourceFile = '') {
  ...
  if (!st.account) recordMissingRequiredField(st.warnings, sourceFile, 'account number');
  if (!st.periodEnd) recordMissingRequiredField(st.warnings, sourceFile, 'statement period');
...
export function parseInvestmentStatements(lines, sourceFile = '') {
  ...
  return { statements: [...byHash.values()], unreadable, warnings };
}
